...
public class MazeGenerator : MonoBehaviour
{
	...
	void Awake()
	{
		...
		if (LevelType == LevelType.CatacombMaze)
		{
			...
			{
				Debug.LogError("Ќеудачный PlaceTorchInCell (стартовый): " + e.ToString() 
					+ "; " + e.ActualValue 
			...
			for (int i = 0; i < 75; i++)
			{
				if (freeCells.Count > 0)
				{
					...
					{
						Debug.LogError("Ќеудачный PlaceTorchInCell: " + e.ToString() + "; " + e.ActualValue + "; freeCells.Count = " + freeCells.Count + "; rnd (index) = " + rnd);
					}
			...
			for (int i = 0; i < _matchboxCount; i++)
			{
				if (cellsForMatchBox.Count > 0)
				{
					...
					{
						Debug.LogError("Ќеудачный cellsForMatchBox: " + e.ToString() + "; " + e.ActualValue + "; cellsForMatchBox.Count = " + cellsForMatchBox.Count);
					}
				}
				else break;
		...
		else if (LevelType == LevelType.ClassicMaze)
		{
			...
			for (int i = 0; i < MazeSize; i++)
				for (int j = 0; j < MazeSize; j++)
				{
					...
					if (j == MazeSize - 1 && i >= 1 && (_mazeMapList[i][j] == 0 || _mazeMapList[i][j] == 2)) // 0 или 2, лень делать бинарное "или", уже и так выше его примен€л
					{
						_exitWall = Instantiate(ExitWallPrefab, _mazeZeroPoint + new Vector3(j * CellSize, 0, -i * CellSize), Quaternion.Euler(0f, -90f, 0f));
					}
				}
	...
	public direction middleOfThree(direction dir1, direction dir2, direction dir3)
	{
		if (dir1 == dir2 || dir2 == dir3 || dir3 == dir1)
			throw new System.ArgumentException("Ќаправлени€ не должны повтор€тьс€!");
		if (dir1 == direction.randomOrUnknown || dir2 == direction.randomOrUnknown || dir3 == direction.randomOrUnknown)
			throw new System.ArgumentException("ѕередано значение randomOrUnknown");
	...
	public void SetWallsHeight(float val)
	{
		...
		foreach (GameObject T in Torches)
		{
			...
			{
				Debug.LogWarning("Ѕаг с факелом! Ќадо исправить!");
			}
		}
	...
	private int manageWall(int where, int which, bool toAdd = true) // 1 пр, 2 лев, 3 всЄ
	{
	...
	private List<List<int>> _generateClassicMaze(int _mazeSize)
	{
		...
		// непосредственно генераци€ при помощи алгоритма Ёллера
		...
		for (int r = 2; r < _mazeSize; r++)
		{
			...
			for (int i = 1; i < _mazeSize; i++)
				if (_mazeMapList[r - 1][i] == 2 || _mazeMapList[r - 1][i] == 3)
					rowSets[i] = i + _mazeSize * r; // присваиваем уникальное множество тем €чейкам, у которых была граница снизу
			...
			{ // создание стенок снизу
				if (rowSets[i] != rowSets[i - 1])
				{
					if (i == _mazeSize - 1 || rowSets[i] != rowSets[i + 1])
					{
						//Debug.Log("Ќе строим стенку дл€ rowSets[" + i + "], = " + rowSets[i]);
						continue;
					}
	...
	public class CatacombMazeMap
	{
		...
		// толща - 1, кра€ - 2. ѕустоты - 0, но их нет изначально
		public CatacombMazeMap(int mazeSize)
		{
			...
			for (int i = 0; i < mazeSize; i++)
			{
				...
				for (int j = 0; j < mazeSize; j++)
				{
					if (i == 0 || i == mazeSize - 1 || j == 0 || j == mazeSize - 1) // внешн€€ граница
		...
		public void DigPathAt(Vector2Int cell)
		{
			if (cell.x > mazeSize - 1 || cell.x < 0 ||
				...
				throw new System.ArgumentOutOfRangeException("cell", cell, "¬ылезли за границы размеров лабиринта");
			...
			else
			{
				...
				throw new System.Exception("“ут уже было пробурено!");
			}
		}
		...
		public void PlaceExitNear(Vector2Int cell)
		{
			if ((cell.x > 1 && cell.x < mazeSize - 2) &&
				...
				throw new System.ArgumentOutOfRangeException("cell", cell , " летка находитс€ в толще, должна быть смежна с краем!");
			...
			else
			{
				if (cell.x == 1)
					...
					throw new System.Exception("¬о врем€ простановки выхода что-то пошло совсем не так");
			}
		}
		...
		public void PlaceLattice(Vector2Int cell)
		{
			if (!catacombMazeMap.GetThinWalls.Contains(cell))
				throw new System.InvalidOperationException("Ёта стена не €вл€етс€ тонкой!");
	...
	private List<List<int>> _generateCatacombMaze(int _mazeSize)
	{
		...
		List<List<int>> _mazeMapList = catacombMazeMap.MazeMap; // толща - 1, кра€ - 2

		// Vector2Int - пор€док x, y
		...
		Debug.Log("¬сего путепроцессов: " + PathProcess.allPathProcesses.Count);
		...
		{
			Debug.LogWarning("Ќеудачный PlaceExitNear: " + e.ToString() + "; " + e.ActualValue + "; PathsEndingAtTheEdge.Count = " + PathsEndingAtTheEdge.Count + "; rndForExit = " + rndForExit);
			return null; // возвращаем null, намека€ на то, что случилс€ фейл
		}
	...
	private class PathProcess
	{
		...
		public void MakeStep()
		{
			...
			while (!directionDetermined && desiredDirections.Count > bannedDirections.Count)
			{
				...
				if (wannaForward && !bannedDirections.Contains(desiredDirections[0])) // то есть "вперЄд"
				{
			...
			{
				// прежде, чем двигатьс€ дальше, нужно решить, хотим ли мы здесь сделать развилку
				...
				if (wannaBranch)
				{
					if (desiredDirections.Count - bannedDirections.Count - 1 > 0)
					{
						...
						{
							// иногда будем делать крестовые перекрЄстки
				...
				{
					Debug.LogError("¬ещаем из catch: " + e.ToString());
			...
			Debug.Log("јктивных путепроцессов: " + activePathProcesses.Count);
		}
	}
